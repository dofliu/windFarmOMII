...
function exportFor(code, { explain = true, hints = 0 } = {}) {
  ...
  record = appendCourseEvent(record, 'LOTO_VERIFIED', { assignmentId: assignment.id, source: 'COURSE_ENGINEERING_LAB', rejectedActions: 2, zeroEnergy: true }, now, assignment);
  record = appendCourseEvent(record, 'WORK_ORDER_CREATED', { assignmentId: assignment.id, source: 'COURSE_ENGINEERING_LAB', rejectedActions: 0, closed: true, lifecycle: ['TRIGGER', 'ACKNOWLEDGE', 'DISPATCH', 'EXECUTE', 'VERIFY', 'CLOSE_OUT'] }, now, assignment);
  for (let index = 0; index < hints; index += 1) record = appendCourseEvent(record, 'HINT_USED', {}, now);
  record = completeCourseAttempt(record, scores, {}, now);
  if (explain) {
    record = updateCourseExplanation(record, { conclusion: 'A', evidence: 'B', uncertainty: 'C', residualRisk: 'D' }, now);
  }
  return JSON.parse(serializeCourseRecord(record, now, { unlockedWeekIds: config.unlockedWeekIds, assignments: config.assignments }));
}
...
describe('Course Record teacher summary', () => {
  ...
  it('builds a learner × week summary, keeps the latest export per code, and renders markdown', () => {
    ...
    expect(markdown).toContain('| OWM-9999-0000 | W01 | MSN-TUT-001 | 1 | 2026-FALL-W01 | 2026-09-08 01:00 | A · 83 | 0/4 | 0 | 3 | 1 (2) | 1 (0) | VERIFIED | DEBRIEF_INCOMPLETE |');
