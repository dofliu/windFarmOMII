...
export interface ReliabilityKpis {
  ...
  uptimeHours: number;
  /** `null` when `observableHours` is 0: undefined, not 0% (worst case). */
  availabilityPercent: number | null;
  /** `null` when there are zero recorded failures: unmeasurable, not 0h (worst case). */
  mtbfHours: number | null;
  /** `null` when there are zero recorded failures: unmeasurable, not 0h (best case). */
  mttrHours: number | null;
  downtimeHours: number;
  lostRevenue: number;
  /** Labor + parts + vessel only; excludes lost revenue (an opportunity cost, not an operating expense). */
  opex: number;
  /** Total cost of the downtime event: lost revenue + OPEX. */
...
export function calculateReliabilityKpis(input: ReliabilityInput): ReliabilityKpis {
  ...
  return {
    ...
    availabilityPercent: observableHours > 0 ? round((uptimeHours / observableHours) * 100, 2) : null,
    mtbfHours: failures > 0 ? round(uptimeHours / failures, 2) : null,
    mttrHours: failures > 0 ? round(Math.max(0, input.repairHours) / failures, 2) : null,
    downtimeHours: round(downtimeHours, 2),
    lostRevenue: round(lostRevenue, 2),
    opex,
    totalDowntimeCost: round(lostRevenue + opex, 2),
  };
}
...
function generateStructuredText(config: AlarmTesterConfig): string {
  return [
    ...
    '(* PersistCounter and AlarmDelay both start from the same first HighCondition sample and run in parallel; *)',
    '(* the alarm sets only once both confirm, matching the trainer trace, not a series/chained timer. *)',
    ...
    `AlarmDelay(IN := HighCondition, PT := T#${Math.max(0, Math.trunc(config.delaySeconds))}s);`,
    'IF ResetCondition THEN',
    '  AlarmActive := FALSE;',
    'ELSIF PersistCounter.Q AND AlarmDelay.Q THEN',
    '  AlarmActive := TRUE;',
    'END_IF;',
