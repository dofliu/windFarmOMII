...
- 現行公開版本為 `3.60.0-course-content-integrity`；保留 Course Record v2 integrity 與 frozen balance、Course 首屏 Quick Start、週次進度列、local-only 完成狀態與預設收合的 Engineering Lab；本輪修復 Engineering Lab 的 OPEX、IEC 61131-3 ST 產生器語意、KPI 零值語意（詳見 `NEXT_SESSION_HANDOFF_2026-09-24.md`）。
...
- 2026-09-24 `pnpm validate` 通過 28 test files／185 tests、Data／Scene／Art／Course、Campaign／Challenge balance 與 production build；`pnpm smoke:course` 另通過 Student Quick Start、Lab disclosure、Assessment no-hints、Course Record v2、390px Mobile 首屏 CTA gate 與更新後的 6 張 Engineering Lab KPI 卡斷言。
